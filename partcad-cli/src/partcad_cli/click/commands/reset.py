...
@click.pass_obj
def cli(ctx, repo_only: bool, sandbox_only: bool, cache_only: bool) -> None:
    with Process("Reset", "global"):
        if repo_only or not (cache_only or sandbox_only):
            ...
            for import_type in cached_import_types:
                cache_dir = os.path.join(ctx.user_config.internal_state_dir, import_type)
        ...
        if sandbox_only or not (repo_only or cache_only):
            sandbox_dir = os.path.join(ctx.user_config.internal_state_dir, "sandbox")
        ...
        if cache_only or not (repo_only or sandbox_only):
            cache_dir = os.path.join(ctx.user_config.internal_state_dir, "cache")
            if os.path.exists(cache_dir):
